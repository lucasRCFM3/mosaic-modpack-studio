...
import { RecommendedPackDialog } from './components/RecommendedPackDialog';
import { RecommendedPacksPage } from './components/RecommendedPacksPage';
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <div className="page-scroll">
        ...
        {view === 'recommendations' ? <RecommendedPacksPage profile={mosaic.currentProfile} feed={mosaic.activeRecommendationFeed} history={mosaic.recommendationHistory} loading={mosaic.loadingRecommendations} loadingId={mosaic.loadingRecommendationId} onLoad={(scope, force) => void mosaic.loadRecommendations(scope, force)} onSelectFeed={mosaic.setActiveRecommendationFeedId} onPreview={(id, desiredModCount) => void mosaic.previewRecommendation(id, desiredModCount)}/> : null}
    ...
    {mosaic.recommendationDetails ? <RecommendedPackDialog details={mosaic.recommendationDetails} profile={mosaic.currentProfile} busy={mosaic.resolvingBatch || mosaic.creatingRecommendedProfile} onClose={() => mosaic.setRecommendationDetails(undefined)} onAdd={(details) => void mosaic.resolveRecommendationForCurrentProfile(details)} onCreate={(details) => void mosaic.createProfileFromRecommendation(details)}/> : null}
