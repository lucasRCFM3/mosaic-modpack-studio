  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <div className="page-scroll">
        ...
        {view === 'modpack' ? <ModpackPage profile={mosaic.currentProfile} onDiscover={() => setView('discover')} onEdit={() => setEditing(true)} onDuplicate={() => setDuplicating(true)} onRemove={(ref) => { if (window.confirm(`Remover ${'name' in ref ? ref.name : 'este mod'} da instância?`)) void mosaic.removeMod(ref); }} onExport={() => void mosaic.exportProfile()} onExportModList={() => void mosaic.exportModList()} onDeleteProfile={() => { if (mosaic.currentProfile && window.confirm(`Remover o perfil “${mosaic.currentProfile.name}”? Os arquivos no disco serão preservados.`)) void mosaic.removeProfile(mosaic.currentProfile.id); }}/> : null}
