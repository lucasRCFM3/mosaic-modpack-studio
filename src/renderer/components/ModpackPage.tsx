...
export function ModpackPage({ profile, classifyingMods, rescanningProfile, onDiscover, onEdit, onDuplicate, onRescan, onOrganize, onRemove, onExport, onExportModList, onDeleteProfile }: {
  profile?: ModpackProfile;
  classifyingMods: boolean;
  rescanningProfile: boolean;
  onDiscover: () => void;
  onEdit: () => void;
  onDuplicate: () => void;
  onRescan: () => void;
  ...
  return <ModpackContent profile={profile} classifyingMods={classifyingMods} rescanningProfile={rescanningProfile} onDiscover={onDiscover} onEdit={onEdit} onDuplicate={onDuplicate} onRescan={onRescan} onOrganize={onOrganize} onRemove={onRemove} onExport={onExport} onExportModList={onExportModList} onDeleteProfile={onDeleteProfile}/>;
}

function ModpackContent({ profile, classifyingMods, rescanningProfile, onDiscover, onEdit, onDuplicate, onRescan, onOrganize, onRemove, onExport, onExportModList, onDeleteProfile }: {
  profile: ModpackProfile;
  classifyingMods: boolean;
  rescanningProfile: boolean;
  onDiscover: () => void;
  ...
  return <div className="page modpack-page">
    <section className="pack-heading"><div><span className="eyebrow"><Icon name="box"/> INSTÂNCIA LOCAL</span><h1>{profile.name}</h1><p>{profile.description || 'Seu modpack modular, reproduzível e verificado.'}</p></div><div className="pack-actions"><button className="button secondary" onClick={onEdit}><Icon name="settings"/> Editar</button><button className="button secondary" onClick={onDuplicate}><Icon name="copy"/> Duplicar</button><button className="button secondary" disabled={rescanningProfile} onClick={onRescan}>{rescanningProfile ? <><span className="spinner"/> Rescaneando…</> : <><Icon name="refresh"/> Rescanear pasta</>}</button><button className="button secondary" disabled={!profile.mods.length || classifyingMods} onClick={onOrganize}>{classifyingMods ? <><span className="spinner"/> Analisando…</> : <><Icon name="layers"/> Separar mods</>}</button><button className="button secondary" onClick={onExportModList}><Icon name="export"/> Gerar TXT</button><button className="button secondary" onClick={onExport}><Icon name="export"/> Exportar lockfile</button><button className="button primary" onClick={onDiscover}><Icon name="plus"/> Adicionar mods</button></div></section>
...
function providerLabel(provider: InstalledMod['provider']) {
  return provider === 'modrinth' ? 'Modrinth' : provider === 'curseforge' ? 'CurseForge' : 'Local';
}
