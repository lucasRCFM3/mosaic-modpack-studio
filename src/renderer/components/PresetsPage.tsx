...
export function PresetsPage({ presets, profile, resolvingPresetId, onCreate, onEdit, onApply, onRemove, onDiscover }: {
  ...
  return <div className="page presets-page">
    <section className="pack-heading"><div><span className="eyebrow"><Icon name="layers"/> BIBLIOTECA REUTILIZÁVEL</span><h1>Predefinições</h1><p>Salve seus mods essenciais uma vez e encontre versões compatíveis para cada novo perfil.</p></div><div className="pack-actions"><button className="button secondary" onClick={onDiscover}><Icon name="compass"/> Descobrir mods</button><button className="button primary" disabled={!profile?.mods.length} onClick={onCreate}><Icon name="plus"/> Criar do modpack atual</button></div></section>
    ...
    </article>)}</div> : <div className="empty-state presets-empty"><span><Icon name="layers"/></span><h3>Nenhuma predefinição ainda</h3><p>Instale seus mods essenciais em um perfil e transforme-os em uma base reutilizável.</p><button className="button primary" disabled={!profile?.mods.length} onClick={onCreate}><Icon name="plus"/> Criar primeira predefinição</button></div>}
  </div>;
}
